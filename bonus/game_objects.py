...
class Snake:
    def __init__(self, game):
        ...
        self.length = 1
        self.segments - []
        self.direction = {pg.K_w: 1, pg.K_s: 1, pg.K_a: 1, pg.K_d: 1}
        
    def control(self, event):
        if event.type == pg.KEYDOWN:
            if event.key == pg.K_w and self.direction[pg.K_w]:
                self.direction = vec2(0, -self.size)
                self.directions = {pg.K_w: 1, pg.K_s: 0, pg.K_a: 1, pg.K_d: 1}
                
            if event.key == pg.K_s and self.direction[pg.K_s]:
                self.direction = vec2(0, self.size)
                self.directions = {pg.K_w: 0, pg.K_s: 1, pg.K_a: 1, pg.K_d: 1}
                
            if event.key == pg.K_a and self.directions[pg.K_a]:
                self.diretion = vec2(-self.size, 0)
                self.directions = {pg.K_w: 1, pg.k_s: 1, pg.K_a: 1, pg.K_d: 0}
                
            if event.key == pg.K_d and self.direction[pg.K_d]:
